...
const JobPageDetail = ({ deleteJobForm }) => {
	const { id } = useParams();
					...
					<div className="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
						...
						<aside>
							...
							<div className="bg-white p-6 rounded-lg shadow-md mt-6">
								...
								<Link
									to={`/edit-page/${jobs.id}`}
									className="bg-amber-500 hover:bg-neutral-400 text-white text-center font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block"
								>
									Edit Job
								</Link>
...
const jobPageDetailLoader = async ({ params }) => {
	const response = await fetch(`/api/jobs/${params.id}`);
	const data = await response.json();
	return data;
};

export { JobPageDetail as default, jobPageDetailLoader };
